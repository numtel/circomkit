...
function instantiateCircuit(config, targetDir, targetPath, usesCustomTemplates) {
  const circuitCode = makeCircuit(config, usesCustomTemplates);
  if (!fs.existsSync(targetDir)) {
    fs.mkdirSync(targetDir, { recursive: true });
  }
  fs.writeFileSync(targetPath, circuitCode);
}
function makeCircuit(config, usesCustomTemplates) {
...
class Circomkit {
  ...
  instantiate(circuit, circuitConfig) {
    ...
    const usesCustomTemplates = circomSource.replace(/\/\/.*|\/\*[\s\S]*?\*\//g, "").includes("pragma custom_templates;");
  ...
  WitnessTester(circuit, circuitConfig) {
    return __async(this, null, function* () {
      ...
      const circomWasmTester = yield circom_tester.wasm(targetPath, {
        output: void 0,
