...
var circom_tester = require('circom_tester');
var fs = require('fs');
...
function downloadPtau(ptauName, ptauDir) {
  ...
  return new Promise((resolve) => {
    https.get(`${PTAU_URL_BASE}/${ptauName}`, (response) => {
      response.pipe(file);
      file.on("finish", () => {
        file.close();
        resolve(ptauPath);
...
class WitnessTester {
  constructor(circomTester) {
    this.circomTester = circomTester;
  }
  ...
  expectConstraintFail(witness) {
    return __async$4(this, null, function* () {
      yield this.expectConstraintPass(witness).then(
        () => {
          throw new node_assert.AssertionError({ message: "Expected constraints to not match." });
        },
        (err) => {
          if (err.message !== "Constraint doesn't match") {
            throw new node_assert.AssertionError({ message: err.message });
          }
        }
      );
    });
  ...
  expectConstraintCount(expected, exact) {
    return __async$4(this, null, function* () {
      ...
      if (count < expected) {
        throw new node_assert.AssertionError({
  ...
  loadSpecificSymbols(symbolsRequested) {
    return __async$4(this, null, function* () {
      ...
      const symbolsFile = yield fs$1.promises.readFile(
      ...
      for (const line of symbolsFile.split("\n")) {
        ...
        symbolsToFind.delete(symbolName);
      }
    });
  }
  /**
...
class ProofTester {
  constructor(wasmPath, pkeyPath, vkeyPath, protocol) {
    ...
    this.verificationKey = JSON.parse(fs.readFileSync(vkeyPath).toString());
  ...
  prove(input) {
    return __async$3(this, null, function* () {
      return snarkjs__namespace[this.protocol].fullProve(input, this.wasmPath, this.pkeyPath, void 0);
    });
  }
  verify(proof, publicSignals) {
    return __async$3(this, null, function* () {
      return yield snarkjs__namespace[this.protocol].verify(
...
function readR1CSInfo(r1csPath) {
  return __async$2(this, null, function* () {
    ...
    const fd = fs.openSync(r1csPath, "r");
  ...
  return __async$1(this, null, function* () {
    ...
    try {
      const result = yield new Promise((resolve, reject) => {
        child_process.exec(`${config.circomPath} ${flags} ${targetPath}`, (error, stdout, stderr) => {
          if (error === null) {
...
function instantiateCircuit(config, targetDir, targetPath) {
  ...
  fs.writeFileSync(targetPath, circuitCode);
}
...
class Circomkit {
  constructor(overrides = {}) {
    ...
    if (this.config.optimization !== void 0 && typeof this.config.optimization !== "number") {
  ...
  readCircuits() {
    return JSON.parse(fs.readFileSync(this.config.circuits, "utf-8"));
  }
  ...
  clear(circuit) {
    return __async(this, null, function* () {
      yield Promise.all([
        promises.rm(this.path.ofCircuit(circuit, "dir"), { recursive: true, force: true }),
        promises.rm(this.path.ofCircuit(circuit, "main"), { force: true })
      ]);
    });
  }
  ...
  vkey(circuit, pkeyPath) {
    return __async(this, null, function* () {
      ...
      const vkey = yield snarkjs__namespace.zKey.exportVerificationKey(pkeyPath, this.snarkjsLogger);
      fs.writeFileSync(vkeyPath, prettyStringify(vkey));
      return vkeyPath;
    });
  }
  ...
  ptau(circuit) {
    return __async(this, null, function* () {
      ...
      if (fs.existsSync(ptauPath)) {
        ...
        fs.mkdirSync(this.config.dirPtau, { recursive: true });
  ...
  contract(circuit) {
    return __async(this, null, function* () {
      ...
      const template = fs.readFileSync(`./node_modules/snarkjs/templates/verifier_${this.config.protocol}.sol.ejs`, "utf-8");
      const contractCode = yield snarkjs__namespace.zKey.exportSolidityVerifier(
        pkey,
  ...
  calldata(circuit, input, pretty) {
    return __async(this, null, function* () {
      const pubs = JSON.parse(
        yield promises.readFile(this.path.ofCircuitWithInput(circuit, input, "pubs"), "utf-8")
      );
      const proof = JSON.parse(
        yield promises.readFile(this.path.ofCircuitWithInput(circuit, input, "proof"), "utf-8")
      );
  ...
  instantiate(circuit, circuitConfig) {
    ...
    const circomSource = fs.readFileSync(`${this.config.dirCircuits}/${circuitConfig.file}.circom`, "utf8");
  ...
  prove(circuit, input, data) {
    return __async(this, null, function* () {
      ...
      const jsonInput = data != null ? data : JSON.parse(fs.readFileSync(this.path.ofCircuitWithInput(circuit, input, "in"), "utf-8"));
      const { proof, publicSignals } = yield snarkjs__namespace[this.config.protocol].fullProve(
        jsonInput,
        wasmPath,
        pkeyPath,
        this.snarkjsLogger
      ...
      yield Promise.all([
        promises.writeFile(this.path.ofCircuitWithInput(circuit, input, "pubs"), prettyStringify(publicSignals)),
        promises.writeFile(this.path.ofCircuitWithInput(circuit, input, "proof"), prettyStringify(proof))
      ]);
      return dir;
    });
  }
  ...
  setup(circuit, ptauPath) {
    return __async(this, null, function* () {
      ...
      if (this.config.protocol === "groth16") {
        ...
        yield snarkjs__namespace.zKey.newZKey(r1csPath, ptauPath, curZkey, this.snarkjsLogger);
        for (let contrib = 1; contrib <= this.config.groth16numContributions; contrib++) {
          ...
          yield snarkjs__namespace.zKey.contribute(
            curZkey,
            nextZkey,
            `${circuit}_${contrib}`,
            this.config.groth16askForEntropy ? void 0 : crypto.randomBytes(32),
            // entropy
            this.snarkjsLogger
          );
          fs.rmSync(curZkey);
        ...
        fs.renameSync(curZkey, pkeyPath);
      } else {
        yield snarkjs__namespace[this.config.protocol].setup(r1csPath, ptauPath, pkeyPath, this.snarkjsLogger);
      }
      const vkey = yield snarkjs__namespace.zKey.exportVerificationKey(pkeyPath, this.snarkjsLogger);
      fs.writeFileSync(vkeyPath, prettyStringify(vkey));
  ...
  verify(circuit, input) {
    return __async(this, null, function* () {
      const [vkey, pubs, proof] = (yield Promise.all(
        ...
        ].map((path) => promises.readFile(path, "utf-8"))
      )).map((content) => JSON.parse(content));
      return yield snarkjs__namespace[this.config.protocol].verify(vkey, pubs, proof, this.snarkjsLogger);
    });
  }
  ...
  witness(circuit, input, data) {
    return __async(this, null, function* () {
      ...
      const jsonInput = data != null ? data : JSON.parse(fs.readFileSync(this.path.ofCircuitWithInput(circuit, input, "in"), "utf-8"));
      fs.mkdirSync(outDir, { recursive: true });
      yield snarkjs__namespace.wtns.calculate(jsonInput, wasmPath, wtnsPath);
      return wtnsPath;
    });
  }
  ...
  input(circuit, input, data) {
    ...
    fs.writeFileSync(inputPath, prettyStringify(data));
    return inputPath;
  }
    ...
    return __async(this, null, function* () {
      ...
      switch (target) {
        ...
        case "r1cs": {
          ...
          json = yield snarkjs__namespace.r1cs.exportJson(path, void 0);
          break;
        }
        // Prover key
        case "zkey": {
          ...
          json = yield snarkjs__namespace.zKey.exportJson(path);
          break;
        }
        // Witness
        case "wtns": {
          ...
          json = yield snarkjs__namespace.wtns.exportJson(path);
          break;
        }
        default:
  ...
  WitnessTester(circuit, circuitConfig, tester = "wasm") {
    return __async(this, null, function* () {
      var _a, _b;
      (_a = circuitConfig.dir) != null ? _a : circuitConfig.dir = "test";
      const targetPath = this.instantiate(circuit, circuitConfig);
      const circomWasmTester = yield (tester === "wasm" ? circom_tester.wasm : circom_tester.c)(targetPath, {
        output: void 0,
        // this makes tests to be created under /tmp
        prime: this.config.prime,
        verbose: this.config.verbose,
        O: typeof this.config.optimization === "number" ? Math.min(this.config.optimization, 1) : void 0,
  ...
  ProofTester(circuit, protocol) {
    return __async(this, null, function* () {
      ...
      const pkeyPath = this.path.ofCircuit(circuit, "pkey");
      const vkeyPath = this.path.ofCircuit(circuit, "vkey");
      const missingPaths = [wasmPath, pkeyPath, vkeyPath].filter((p) => !fs.existsSync(p));
